...
tryBind = (return (let tryBind' x y = ((((return y)>>=( \ x3  -> (((return x)>>=( \ x4  -> (plus>>=( \ g4  -> (g4 x4)))))>>=( \ g3  -> (g3 x3)))))>>=( \ x1  -> ((return return)>>=( \ g1  -> (g1 x1)))))>>=( \ z  -> ((return z)>>=( \ x1  -> ((return return)>>=( \ g1  -> (g1 x1)))))));;in (mConvert1 tryBind')));
result :: (Eff r (Integer,Integer) )
result = (let result' = ((return 2)>>=( \ s0  -> ((runState s0) (addState'>>=( \ g1  -> (g1 ((return 5)>>=( \ x2  -> (((return 4)>>=( \ x3  -> (tryBind>>=( \ g3  -> (g3 x3)))))>>=( \ g2  -> (g2 x2)))))))))));;in result');
